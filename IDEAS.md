...
> **Higher-value follow-up (5a) — "configured-but-unenforced field" lint:** a rule that warns when a KDL field parses but has no runtime effect (a silent no-op) would catch a real operator hazard. Suspected members: `pool.*` (the whole agent `pool { }` block — `connect-timeout-ms`, `chunk-timeout-ms`, `connections-per-agent`, …), and `cipher_suites` (already flagged by semantic validation).
>
> **Blocked on verification, not effort.** A grep sweep of `crates/proxy/src` found no consumers for the agent `pool` fields *or even* per-agent `timeout-ms` — but the agent config→runtime path flows through a type conversion/builder, so a literal-name grep cannot see the consumption and **empty grep ≠ unenforced**. A lint that asserts "this field does nothing" on grep evidence risks the *inverse* cipher trap: telling operators a field is dead when it is wired via indirection. Building 5a correctly therefore requires (1) a real data-flow trace (code-graph / `trace_path`, not grep) to produce a trustworthy catalog of wired-vs-unwired fields, and (2) a regression test that fails when a listed field gains a consumer, so the catalog cannot rot into false positives. That is a dedicated audit, deliberately not attempted as a quick slice.
